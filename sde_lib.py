    ...
    def reverse(self, score_fn, probability_flow=False):
        ...
        class RSDE(self.__class__):
            ...
            def discretize(self, x, t, atb, csm, atb_mask):
                ...
                meas_grad *= torch.norm(grad)
                meas_grad *= self.config.sampling.mse
                if config.training.sde == "vesde" and config.sampling.method == "cg":
                    f, G, sigma, adjacent_sigma = discretize_fn(x, t)
                    score = score_fn(x, t)
                    return sigma, adjacent_sigma, score
                else:
                    f, G, sigma, adjacent_sigma = discretize_fn(x, t)
...
class VESDE(SDE):
    ...
    def discretize(self, x, t):
        ...
        return f, G, sigma, adjacent_sigma
...
class SpiritSDE(SDE):
    ...
    def reverse(self, score_fn, kernel, probability_flow=False):
        """Create the reverse-time SDE/ODE.
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, x, t, atb, kernel, atb_mask, csm):
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                sigma = self.sigma_min * (self.sigma_max / self.sigma_min) ** t
                drift = self.eta / 2 * Psi(x, kernel)
                diffusion = sigma * torch.sqrt(
                    torch.tensor(
                ...
                condition = grad - meas_grad
                VU_condition = (
                    orthogonal_csm(csm, condition)
                    .type(torch.FloatTensor)
                    .to(config.device)
                )
                VU_condition = (
                    orthogonal_csm(csm, VU_condition)
                    .type(torch.FloatTensor)
                    .to(config.device)
                )
                drift = drift - diffusion[:, None, None, None] ** 2 * VU_condition * (
                    0.5 if self.probability_flow else 1.0
                )
                # Set the diffusion function to zero for ODEs.
                diffusion = 0.0 if self.probability_flow else diffusion
                ...
                z = orthogonal_csm(csm, z)

                x_mean = x + drift * dt
                x = x_mean + diffusion[:, None, None, None] * np.sqrt(-dt) * z
                return x, x_mean

            def discretize(self, x, t, atb, kernel, atb_mask, csm):
                """Create discretized iteration rules for the reverse diffusion sampler."""
                grad = score_fn(x, t)
                meas_grad = Emat_xyt(x, False, None, atb_mask) - c2r(atb)
                meas_grad = Emat_xyt(meas_grad, True, None, atb_mask)
                ...
                rev_f = f - G[:, None, None, None] ** 2 * VU_condition * (
                    0.5 if self.probability_flow else 1.0
                )
                ...
                x_mean = x - rev_f
                x = x_mean + rev_G[:, None, None, None] * z
