    ...
    def forward(self, x, time_cond):
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                if h.shape[-1] in self.attn_resolutions:
                    ...
                    m_idx += 1
                hs.append(h)
                # print('h.shape=',h.shape,'hs.shape=',hs[-1].shape)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                tmp = hs.pop() # return and remove the last element from hs
                # Odd down-sampling sizes need explicit output padding.
                
                h = F.interpolate(h, size=(tmp.shape[-2],tmp.shape[-1]), mode='bilinear', align_corners=False)
            ...
            if self.progressive != "none":
                if i_level == self.num_resolutions - 1:
                    ...
                    if self.progressive == "output_skip":
                        ...
                        pyramid = F.interpolate(pyramid, size=(pyramid_h.shape[-2],pyramid_h.shape[-1]), mode='bilinear', align_corners=False)
        ...
        if self.config.model.scale_by_sigma:
            if self.config.data.dataset_name == "VWI":
                # TODO: 这里应该不用复制18份也没事?
                used_sigmas = used_sigmas.reshape((1, *([1] * len(x.shape[1:]))))
            else:
